...
<s:portlet url="/pl/goods-list.htm" var="goods" param="flag_recommend=0&orderbyIndex=4"/>
...
<jsp:useBean id="menu" class="java.lang.String"/><c:set var="menu" value="c-recommend"/>
        ...
        <div id="content">
            ...
            <c:forEach var="unit" items="${goods.list}" varStatus="status">
                <c:if test="${param.editingId==unit.id}">
                    ...
                    <tags:list-add list="${rowData}"><input type="text" name="flag_recommend" size="3" value="${unit.flag_recommend}"></tags:list-add>
                    <tags:list-add list="${rowData}">
                        <tags:btn-common id="okBtn_${unit.id}" iconClass="ui-icon-check" label="确定" actionScript="$('#id').val(${unit.id});$('#toUrl').val(buk_replaceQueryString(window.location.href, 'editingId', null));$('#form').submit()"/>
                        <tags:btn-common id="cancelBtn_${unit.id}"  iconClass="ui-icon-close" label="取消" actionScript="window.location.href=buk_replaceQueryString(window.location.href, 'editingId', null)"/>
                    </tags:list-add>
                </c:if>
                <c:if test="${param.editingId!=unit.id}">
                    <tags:list-add list="${rowData}">${status.count}</tags:list-add>
                    <tags:list-add list="${rowData}">${unit.id}</tags:list-add>
                    <tags:list-add list="${rowData}">${unit.title}</tags:list-add>
                    <tags:list-add list="${rowData}">${unit.flag_recommend}</tags:list-add>
                    <tags:list-add list="${rowData}">
            ...
            <c:if test="${empty param.editingId}">
                ...
                <tags:list-add list="${rowData}"> - </tags:list-add>
                <tags:list-add list="${rowData}"><input type="text" name="flag_recommend"  size="3"></tags:list-add>
