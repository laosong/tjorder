...
<s:portlet url="/pl/goods-list.htm" var="pvHigh" param="state=2&types=1&pageNo=1&pageItemNum=3&orderbyIndex=11"/>
<s:portlet url="/pl/goods-list.htm" var="pList"
           param="state=2&types=1&pageItemNum=8&pageNo=${param.pageNo}&orderbyIndex=${oIndex}${filter1}${filter2}"/>
<html>
<head>
        ...
        <div class="product m790 right">
            <div class="p_brand">
                ...
                <dl class="mart10 clearfix">
                    <dt>品牌</dt>
                    <dd>
                        <a href="javascript:util_setHref(util_replaceUrl('list.htm'));" <c:if test="${empty bId}">class="curB"</c:if> >不限</a>
                        <c:forEach var="unit" items="${root.currNode.children}" varStatus="status">
                            <a href="javascript:util_setHref(util_replaceUrl('list${unit.data.id}.htm'));" <c:if test="${unit.data.id==bId}">class="curB"</c:if> >${unit.data.full_name}</a>
                        </c:forEach>
                    </dd>
                </dl>
                <c:if test="${!empty bId}">
                    <dl class="mart10 clearfix">
                        <dt>系列</dt>
                        <dd>
                            <a href="javascript:util_setHref(util_replaceUrl('list${bId}.htm'));" <c:if test="${sId==0}">class="curB"</c:if> >不限</a>
                            <c:forEach var="unit" items="${brand.currNode.children}" varStatus="status">
                                <a href="javascript:util_setHref(util_replaceUrl('list${unit.data.id}.htm'));" <c:if test="${unit.data.id==sId}">class="curB"</c:if> >${unit.data.full_name}</a>
                            </c:forEach>
                        </dd>
                    </dl>
                </c:if>
                <dl class="noborder clearfix">
                    <dt>价格${pIndex}</dt>
                    <dd>
                        <a href="javascript:util_setHref(util_replaceQString3(util_replaceQString2('p1',null),'p2',null));" <c:if test="${pIndex==0}">class="curB"</c:if>>不限</a>
                        <a href="javascript:util_setHref(util_replaceQString3(util_replaceQString2('p1',1),'p2',99));" <c:if test="${pIndex==1}">class="curB"</c:if>>1-99元</a>
                        <a href="javascript:util_setHref(util_replaceQString3(util_replaceQString2('p1',100),'p2',299));" <c:if test="${pIndex==2}">class="curB"</c:if>>100-299元</a>
                        <a href="javascript:util_setHref(util_replaceQString3(util_replaceQString2('p1',300),'p2',599));" <c:if test="${pIndex==3}">class="curB"</c:if>>300-599元</a>
                        <a href="javascript:util_setHref(util_replaceQString3(util_replaceQString2('p1',600),'p2',999));" <c:if test="${pIndex==4}">class="curB"</c:if>>600-999元</a>
                        <a href="javascript:util_setHref(util_replaceQString3(util_replaceQString2('p1',1000),'p2',1999));" <c:if test="${pIndex==5}">class="curB"</c:if>>1000-1999元</a>
                        <a href="javascript:util_setHref(util_replaceQString3(util_replaceQString2('p1',2000),'p2',999999));" <c:if test="${pIndex==6}">class="curB"</c:if>>2000元以上</a>
                    </dd>
                </dl>
            </div>
            <div class="p_order mart10">
                <dl class="clearfix">
                    ...
                    <dd class="left">
                        ...
                        <a href="javascript:util_setHref(util_replaceQString2('o',${oo2}));" <c:if test="${oIndex==23||oIndex==24}">class="curO"</c:if> >价格 <c:if test="${oIndex==24}">↓</c:if> <c:if test="${oIndex==23}">↑</c:if></a>
