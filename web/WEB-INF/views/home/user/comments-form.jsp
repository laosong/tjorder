...
<s:portlet var="goodsObj" url="/pl/goods-obj.htm" param="id=${param.gid}"/>
        ...
        <div class="personal_con m790 right">
            ...
            <div class="pCmt_del tablestyle mart10">
                ...
                <div class="pCmt_p clearfix">
                    <div class="pCmt_p_img left"><a href="/home/product/${goodsObj.goods.id}.htm" target="_blank" title="${goodsObj.goods.degree}° ${goodsObj.goods.title}${goodsObj.goods.volume}ml"><img src="${upfilePl.map['l-default'].path}" alt="${goodsObj.goods.degree}° ${goodsObj.goods.title}${goodsObj.goods.volume}ml" /></a></div>
                ...
                <form:form id="commentsForm" commandName="cmd" action="/home/user/add-comments.htm" method="post">
                   <br/>
                    ...
                    <div class="pCmt_area">
                        <label  class="f14px fb"><span>*</span>满意度打分：</label>
                        <ul class="pCmt_stars mart10 clearfix">
                            <li><input type="radio" name="score" value="5" checked="checked"/><div class="btn cmt_stars stars_5">5星</div>(5分)</li>
                            <li><input type="radio" name="score" value="4" /><div class="btn cmt_stars stars_4">4星</div>(4分)</li>
                            <li><input type="radio" name="score" value="3" /><div class="btn cmt_stars stars_3">3星</div>(3分)</li>
                            <li><input type="radio" name="score" value="2" /><div class="btn cmt_stars stars_2">2星</div>(2分)</li>
                            <li><input type="radio" name="score" value="1" /><div class="btn cmt_stars stars_1">1星</div>(1分)</li>
                        </ul>
                    </div>
                    <div class="pCmt_area mart20">
                        <label for="comment" class="f14px fb"><span>*</span>评语：</label>
                        <textarea rows="6" id="comment" name="msg" class="pCmt_textarea mart5"></textarea>
                        <input type="hidden" name="owner_id" value="${goodsObj.goods.id}"/>
                        <input type="hidden" name="creator" value="${u.user.id}"/>
                    </div>
                    <div class="pCmt_submit link_btn mart10"><a href="javascript:$('#commentsForm').submit();" class="btn">提交</a></div>
                </div>
                </form:form>
            </div>
        </div>
